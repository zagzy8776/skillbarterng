        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="relative bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 dark:from-indigo-700 dark:via-purple-700 dark:to-pink-700 rounded-3xl p-8 mb-12 shadow-2xl overflow-hidden">
            ...
            <div className="relative flex flex-col lg:flex-row items-center justify-between">
              ...
              <div className="flex flex-wrap justify-center lg:flex-nowrap gap-6 lg:space-x-8">
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-emerald-600 dark:text-emerald-400">{totalConnections}</div>
                  <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Connections</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-blue-600 dark:text-blue-400">{activeSwaps}</div>
                  <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Active Swaps</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-indigo-600 dark:text-indigo-400">{profileCompletion}%</div>
                  <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Profile Complete</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-16">
            {popularSkills.map((skill, i) => (
              <div key={i} className="bg-gradient-to-br from-pink-50 via-purple-50 to-pink-100 rounded-2xl p-4 sm:p-6 text-center border border-pink-200 shadow-lg hover:shadow-xl transition transform hover:scale-105">
                <h3 className="text-sm sm:text-lg lg:text-xl font-semibold text-gray-900 mb-3 sm:mb-4">{skill.name}</h3>
                <p className="text-green-600 text-sm sm:text-base lg:text-lg">{skill.offer} people dey teach am</p>
                <p className="text-pink-600 text-sm sm:text-base lg:text-lg">{skill.want} people wan learn am</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-12 sm:mb-16">
            {filteredMatches.length > 0 ? filteredMatches.map((match) => (
              <div key={match.id} className="bg-white rounded-2xl p-4 sm:p-6 lg:p-8 text-center border border-gray-300 shadow-lg hover:shadow-xl transition transform hover:scale-105 group">
                <div className="relative">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-gradient-to-br from-purple-400 to-pink-400 rounded-full mx-auto mb-3 sm:mb-4 flex items-center justify-center text-white text-lg sm:text-xl lg:text-2xl font-bold">
                    {match.name.split(' ').map(n => n[0]).join('')}
                  </div>
                  <div className="absolute -top-1 -right-1 sm:-top-2 sm:-right-2 w-4 h-4 sm:w-6 sm:h-6 bg-green-500 rounded-full border-2 border-white flex items-center justify-center">
                    <span className="text-xs text-white">✓</span>
                  </div>
                </div>
                <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-1 sm:mb-2">{match.name}</h3>
                <p className="text-green-600 mb-3 sm:mb-4 text-xs sm:text-sm">{match.university}</p>
              ...
              <div className="col-span-full text-center">
                ...
                <Link
                  href="/invite"
                  className="inline-block px-6 sm:px-8 py-3 bg-purple-500 hover:bg-purple-600 text-white font-bold rounded-full transition shadow-lg hover:shadowscale-105 text-xl transform hover:-sm sm:text-base"
                >
                  Invite Friends 🚀
                </Link>
              </div>
